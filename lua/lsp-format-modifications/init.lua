...
local base_config = {
	...
	diff_callback = function(comparee_content, buf_content)
		return vim.diff(comparee_content, buf_content, M.default_diff_opts)
	end,
...
M.format_modifications = function(lsp_client, bufnr, config)
	...
	if err ~= nil then
		util.notify("failed checks: " .. err, vim.log.levels.ERROR)
	...
	if err ~= nil then
		util.notify(err .. ", doing nothing", vim.log.levels.WARN)
		return { success = false }
	end
	...
	if err ~= nil then
		util.notify("failed to get comparee, " .. err .. " -- consider raising a GitHub issue", vim.log.levels.ERROR)
		return { success = false }
	end
...
M.format_modifications_buffer = function(bufnr)
	...
	if ctx == nil then
		...
		util.notify("no supported LSP clients attached to buffer, nothing to do", vim.log.levels.WARN)
		return { success = false }
	end
...
M.attach = function(lsp_client, bufnr, provided_config)
	...
	if config.format_on_save then
		local augroup_id =
			vim.api.nvim_create_augroup("NvimFormatModificationsDocumentFormattingGroup", { clear = false })
	...
	vim.api.nvim_create_user_command("FormatModifications", M.format_modifications_current_buffer, {})
